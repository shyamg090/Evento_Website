    ...
    return (
        <div className="flex flex-col items-center justify-center gap-8 py-8 bg-gray">
            ...
            <div className="lg:w-[80%] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-rows-none place-items-center gap-4">
                ...
                {
                    getevents.map((item, id) => {
                        return <div key={id} className="flex flex-col min-w-[25rem] hover:shadow-2xl hover:rounded-md p-2">

                            <div className="w-full h-[30vh] bg-cover bg-center" style={{ backgroundImage: `url(${eventimage})` }}></div>

                            <div className="p-4 flex flex-col gap-2">
                                <h1 className="text-[1.5rem]"> eventname </h1>
                                <div className="flex items-center justify-start gap-4">
                                    <h1 className="flex  items-center justify-center gap-2"> <FaLocationDot className="text-red" /> location</h1>
                                    <h1 className="flex items-center justify-center  gap-2"> <BsCalendarDateFill className="text-red" /> date</h1>
                                    <h1 className="flex  items-center justify-center gap-2"> <IoIosTime className="text-red" /> time</h1>
                                </div>
                                <h1>hostedby</h1>
                                <h1>startingfrom: price</h1>
                                <h1 className="bg-red-600 w-[5rem] rounded-md text-white text-center text-[.8rem] p-1">category</h1>
                            </div>
                        </div>
                    })
                }
